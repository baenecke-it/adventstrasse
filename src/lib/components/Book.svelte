...
<div
	...
	style="--cover-color: {coverColor};
                --book-shadow: {shadow === 2
		? '-10px 0 50px 10px #666'
		: shadow === 1
			? '5px 5px 20px #666'
			: 'none'};
...
>
	<div class="book">
		{#if coverImage}
			<img class="cover front" alt={coverImageAlt} src={coverImage} />
		{:else}
			<div class="cover front text">{coverImageAlt}</div>
		{/if}
		{#if spineImage}
			<img class="spine" alt={spineImageAlt} src={spineImage} />
		{:else}
			<div class="spine text">{spineImageAlt}</div>
		{/if}
		{#if backCoverImage}
			<img class="cover back" alt={backCoverImageAlt} src={backCoverImage} />
		{:else}
			<div class="cover back text">{backCoverImageAlt}</div>
		{/if}
	</div>
</div>
